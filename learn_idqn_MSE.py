...
def train_qnetwork(samples, criterion,qnetName):
    ...
    for epoch in range(EPOCHS):  # loop over the dataset multiple times
        ...
        if epoch == 0:
            trainingEvaluations[epoch] = runTest(10,epoch,None)
            torch.save(net.state_dict(), f'artifacts/models/{lossFn}/{lossFn}_mode20_agent_1{epoch}.pt')

        if epoch> 0 and epoch <= 100:
            torch.save(net.state_dict(), qnetName)
            torch.save(net.state_dict(), f'artifacts/models/{lossFn}/{lossFn}_mode20_agent_1{epoch}.pt')
            trainingEvaluations[epoch] = runTest(10,epoch,qnetName)
    ...
    return net, trainingEvaluations
...
if __name__ == '__main__':
    ...
    wandb.init(project="iDQN",name=f"mode_20_{lossFn}_loss")
    agent_samples = retrieveRandomFromMongo(25000, 1, lossFn, query=None)
    net, trainingEvaluations = train_qnetwork(agent_samples, criterion,qnetName)
